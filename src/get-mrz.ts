...
export interface MrzOptions<Debug = false> {
  debug?: Debug;
  out?: Partial<GetMrzDebugResult>;
}

export interface GetMrzDebugResult {
  resized?: Image;
  grey?: Image;
  gaussian?: Image;
  blackhat?: Image;
...
function internalGetMrz(image: Image, options?: MrzOptions): GetMrzResult;
function internalGetMrz(
  image: Image,
  options?: MrzOptions<true>,
): GetMrzDebugResult;
function internalGetMrz(
...
function internalGetMrz(
  ...
  image = image.scharrFilter({
    direction: 'x' as GradientDirection,
    bitDepth: 32,
  });
  image = image.abs();
  ...
  image = image.mask({
    algorithm: 'otsu' as ThresholdAlgorithm,
  });
  ...
  let rawRois = roiManager.getRois({
    minSurface: 5000,
    // minWidth: 400
  });

  let masks = rawRois.map((roi) => roi.getMask());
  let rois = rawRois.map((roi, idx) => {
  ...
  if (Math.abs(angle) < 1) {
    ...
    const afterRotate = beforeRotate.rotate(angle, {
      interpolation: 'bilinear' as InterpolationAlgorithm,
    });
...
function checkRatio(ratio: number) {
...
function getDiffVector(p1: number[], p2: number[]) {
